#requests 모듈
#requests는 python에서 HTTP 요청을 보내는데 사용되는 라이브러리
import requests
#
# response = requests.get("https://www.naver.com/")
# print(response.status_code) #웹 서버로부터 받은 응답의 HTTP 상태 코드를 출력(200이면 정상 출력)
# print(response.text)

#pandas 모듈
#pandas는 데이터 조작 및 분석을 위한 라이브러리
#csv, excel, 데이터 베이스 등 다양한 형식의 데이터를 읽고 쓸 수 있으며,
...
#matplotlib
#python에서 정적인, 애니메이션화된, 그리고 상호작용하는 시각화를 만드는데 사용되는 라이브러리
...
#빠르고 효율적인 배열 연산을 제공하여 과학, 공학 분야에서 수학적 계산을 수행하는데 필수적이다.
#벡터, 행렬 등 선형 대수학 연산에 최적화되어 있다.
import numpy as np
